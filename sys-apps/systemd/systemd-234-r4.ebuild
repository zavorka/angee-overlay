...
IUSE="acl apparmor audit build cryptsetup curl elfutils +gcrypt gnuefi http
	idn importd +kmod libidn2 +lz4 lzma nat openvz pam policykit
...
src_prepare() {
	...
	if use openvz; then
		PATCHES+=(
			"${FILESDIR}/234-0005-openvz-cgroup-xattrs-invocation-id.patch"
                        "${FILESDIR}/234-0006-openvz-ipv6.patch"
                        "${FILESDIR}/234-0007-openvz-keyring.patch"
                        "${FILESDIR}/234-0008-openvz-libdir.patch"
                        "${FILESDIR}/234-0009-openvz-namespaces.patch"
                        "${FILESDIR}/234-0010-openvz-udev-rules-kmod.patch"
		)
	fi
